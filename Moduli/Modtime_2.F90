...
subroutine modulis(ins,outs,dim1,dim2)
	implicit none
		...
		real									:: d_freq,ff ! Change in frequency value, used in fill
...
om_i = -5
om_f = 0
d_freq = (om_f - om_i)/(5000)

freq_fill: do i = 1, 5000, 1
	ff = om_i + float(i-1)*d_freq
        freq(i)=10**ff
end do freq_fill
...
freq_loop: do i = 1, 5000, 1
	...
	freq_mod = (cmplx(1.0,0.0) - cexp(cmplx(0.0,-freq_i*ins(1,1)))) * &
						 cmplx(ins(1,2),0.0)/cmplx(ins(1,1),0.0) + &
	 					 (2*Diff_const)*cexp(cmplx(0.0,-freq_i*ins(dim1,1))) + sum_in
